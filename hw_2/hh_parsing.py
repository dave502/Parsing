...
class HHScraper:
    ...
    def __get_salary_min_max(self, salary_text: str) -> list:
        ...
        if salary_text:
            ...
            if start_max > start_min:
                salary_max = salary_text[start_max + len('до'):re.search(r'\d+', salary_text).end()]
            else:  # в остальных слуаях ищем все числа, первое - минимальная зарплата, второе - максимальная
                digits = re.findall(r'\d+', salary_text)
                salary_min = digits[0] if len(digits) > 0 else None
                salary_max = digits[1] if len(digits) == 2 else None
    ...
    def get_jobs_list(self):
        ...
        job_data_list = []
        self.__params = {'text': self.__input_vacancy_text()}
